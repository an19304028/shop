...
public class MySQLGetItemDetailDao implements GetItemDetailDao{
	...
	public List getItemDetail(String key) {
		...
		try {
			...
			String sql = "SELECT i.item_id, i.item_name, i.stock_count, s.size_name ,c.color_name, ca.category_name, i.price, i.detail, c.color_image_path FROM shop.item_table i JOIN shop.size_table s USING(size_id) JOIN shop.color_table c USING(color_id) JOIN shop.category_table ca USING(category_id) Where item_name=?";
			...
			while(rs.next()) {
				...
				i.setColorImagePath(rs.getString(9));
