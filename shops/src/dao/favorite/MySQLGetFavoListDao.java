...
public class MySQLGetFavoListDao implements GetFavoListDao{
	...
	public List getFavoList(String userId) {
		...
		try {
			...
			String sql = "SELECT i.item_id, i.item_name, i.stock_count, s.size_name ,c.color_name, i.price, ca.category_name, i.detail, im.image_path FROM shop.favo_table JOIN item_table i USING(item_id) JOIN shop.size_table s USING(size_id) JOIN shop.color_table c USING(color_id) JOIN shop.category_table ca USING(category_id) JOIN shop.image_table im USING(item_id) WHERE user_id=?";
